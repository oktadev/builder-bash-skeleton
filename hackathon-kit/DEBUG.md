# DEBUG — 28 failure shapes

Match your symptom, apply the fix. Every row carries the path it applies to, and
the sections group them — the **MCP** section is the only `APP_TYPE=mcp`-only
one. IDs are stable, so they aren't in numeric order within a section; scan by
symptom, not by number. If nothing matches, the curl recipes at the bottom
isolate a client bug from a server/registration bug. After ~5 failed attempts on
one issue, stop and report.
...
| D-24 | both | The refresh token is visible when you base64-decode your own session cookie | Your session library **signs but doesn't encrypt** (Starlette `SessionMiddleware`, `itsdangerous`, `gorilla/sessions` without a block key, Rails' cookie store). Signed ≠ encrypted: the payload is readable by anyone holding the cookie. Nothing in the kit's tests catches it except T9.1. Move to a server-side store or genuine authenticated encryption — SPEC § Invariants 3 lists a working option per stack. |
...
| D-25 | SAML | Assertion validation passes, but you extracted the assertion by slicing the raw `SAMLResponse` | Verify the **signed `Reference URI` equals the `ID` of the element you sliced** (validation 2). Libraries hand back a re-serialised assertion, so slicing original bytes is correct — but without that comparison you've hand-built the signature-wrapping vulnerability the kit warns about. Also: the prefix isn't always `saml:`; `saml2:` is common. |
| D-26 | SAML | `NotBefore`/`NotOnOrAfter` rejected on a clock that looks fine | Your SAML library's skew tolerance defaults to **zero** — `@node-saml/node-saml` does. Set 30–60 s explicitly. Looks like D-18 but is a different layer: D-18 is the ID-JAG `iat` at Step 2, this is the assertion `Conditions` at Step 0. |
...
| D-23 | mcp | Discovery requests your code never made; a DCR `POST`; a redirect-to-authorization; an `UnauthorizedError` from the transport; **or a connection failure naming `http://authorization-server:5001`** | **Architecture bug, not config** — *unless it fired on an expired token, which is routine; see D-27.* That last symptom is unmistakeable — an unroutable internal Docker hostname leaked by `mcp.xaa.dev`'s own AS metadata, reachable only by following the discovery chain. MCP's built-in OAuth (RFC 9728 → DCR → auth-code + PKCE) *competes* with XAA; here the token exists before the client connects. Supply it via the `authProvider` seam, leave acquisition members unimplemented (BUILD § 3b). The SDK only reaches for OAuth from a 401 handler — so either a 401 happened (fix that: D-20) or your provider permits acquisition. Letting it run registers a **third** client identity. On a 401 the correct move is the kit's error mapping, never handing control to the SDK. |
| D-27 | mcp | "MCP SDK attempted its own OAuth" fires after the app has been running a couple of hours, on a build that was working | **Not D-23 — this is the ~2 h access token expiring.** The SDK 401-handles, reaches your provider's guard, and throws. Catch that error type in Phase 4 and treat it as the 401 it came from: `expired_token`/`step3`, then re-mint and retry **once** (BUILD § 3b). Only treat it as D-23 if it fires with a **freshly minted** token. Letting it escape raw is why a routine expiry looks like an architecture failure. |
| D-28 | mcp | Second and later `/api/call`s fail, or use a token you know is stale | You reused one `Client`/transport across calls. The `authProvider` closes over a **single** access token, so a long-lived client keeps presenting an expired one — and caching it violates Invariant 7. Build a fresh transport + `Client` per call and `close()` it; the repeated `initialize` is expected. |
